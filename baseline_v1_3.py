...
class Model():
    def __init__(self,input_tensor,position_tensor,sequence_num,drop_rate):
    ...
    def built_model(self):
        ...
        with tf.variable_scope("lstm"):
            ...
            lstm_bw_cell =[]
            for _ in range(3):
                lstm_fw_cell.append(tf.contrib.rnn.BasicLSTMCell(HIDDEN_SIZE,forget_bias =1.0))
                lstm_bw_cell.append(tf.contrib.rnn.BasicLSTMCell(HIDDEN_SIZE,forget_bias = 1.0))
            ...
            output,fw_state,bw_state = tf.contrib.rnn.static_bidirectional_rnn(mul_lstm_fw_cell,mul_lstm_bw_cell,x,dtype=tf.float32,sequence_lenth = self.sequence_num)
            # lstm_concat = tf.concat(output,axis=-1)
            # lstm_concat = tf.transpose(lstm_concat,[1,0,2])
            self.lstm_output = tf.concat([fw_state[-1].h,bw_state[-1].h],-1)
...
def train():
    ...
    Sequence_lenth = tf.placeholder(dtype = tf.int32,shape = [None])
    Drop_rate = tf.placeholder(tf.float32)
    Output = tf.placeholder(dtype=tf.int32,shape=[None])
    model = Model(Input,Position,Sequence_lenth,Drop_rate)
    ...
    train_data_loader = Data_loader(SEQUENCE_NUM,MAX_DEPTH,BATCH_SIZE,TRAIN_PATH,r'tf_data/test/')
    test_ans,test_conj_f,test_conj_p,test_conj_sl,test_stat_f,test_stat_p,test_conj_sl = test_data_loader.read_and_decode()
    train_ans,train_conj_f,train_conj_p,train_conj_sl,train_stat_f,train_stat_p,train_stat_sl = train_data_loader.read_and_decode()
    label_batch, conj_f_batch, conj_p_batch,conj_sl_batch, stat_f_batch, stat_p_batch,stat_sl_batch = tf.train.shuffle_batch([train_ans,train_conj_f,train_conj_p,train_stat_f,train_stat_p],BATCH_SIZE,BATCH_SIZE*4,BATCH_SIZE*2,32)
    test_label_batch,test_conj_f_batch,test_conj_p_batch,test_conj_sl_batch,test_stat_f_batch,test_stat_p_batch,test_stat_sl_batch = tf.train.batch([test_ans,test_conj_f,test_conj_p,test_stat_f,test_stat_p],TEST_BATCH_SIZE,32)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(EPOCH):
            for mini_step in range(2040000//BATCH_SIZE):
                label_data,stat_f_data,stat_p_data,stat_sl_data = sess.run([label_batch,stat_f_batch,stat_p_batch,stat_sl_batch])
                if mini_step==0:
                    print("successful")
                train_acuracy,train_loss,_ = sess.run([correct,loss,optimizer],feed_dict={Input:stat_f_data,Position:stat_p_data,Sequence_lenth:stat_sl_data,Output:label_data,Drop_rate:0.5})
                if mini_step%1000==0:
            ...
            for index in range(200000//TEST_BATCH_SIZE):
                test_label_data,test_stat_f_data,test_stat_p_data,test_stat_sl_data =sess.run([test_label_batch,test_stat_f_batch,test_stat_p_batch,test_stat_sl_batch])
                test_accuracy,test_loss = sess.run([correct,loss],feed_dict={Input:test_stat_f_data,Position:test_stat_p_data,Sequence_lenth:test_stat_sl_data,Output:test_label_data,Drop_rate:0.0})
